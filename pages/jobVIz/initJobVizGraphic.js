  ...
  var margin = { top: 0, right: 60, bottom: 0, left: 60 },
    width = Math.max(window.innerWidth, 1800) - margin.right - margin.left,
    height = window.innerHeight - margin.top - margin.bottom;
  const initJobVizGraphic = () => {
    console.log("we loaded initJobVizGraphic");
    ...
    $.getJSON("/job_tree_v2.json", function (treeData) {
      ...
      tree_loaded = true;
      root = treeData[0];
      small_root = jQuery.extend(true, {}, treeData[0]);
      small_stack = [jQuery.extend(true, {}, treeData[0])];
      root.x0 = height / 2;
      root.y0 = 0;
    ...
    function showSmallNode(node) {
      ...
      var job = data[0];
      console.log(parseFloat(job["Employment.2016"]));
